...
- **Compact action palette** — Press ⌘K anytime for a focused command sheet, then keep typing to filter actions without polluting the main search box
- **Always-discoverable global actions** — Even with an empty history or no current selection, the bottom bar and empty states still surface `⌘K` so settings and global commands never disappear
...
| `⌘K` | Toggle action palette, even when no item is selected |
